...
echo "Error: '$formatted' Command doesn't exist."
    local choice=""
    read -p "Do you want tip? (Y/N): " choice
    [[ -z "$choice" ]] && choice="Y"
    case $choice in
        [Yy])
	    # Call OpenAI API
	  ...
	  local response=$(curl -s  --request POST \
	     ...
	     --data "{\"model\":\"gpt-3.5-turbo\", \"max_tokens\": 100,\"temperature\": 0.7,\"messages\": [{\"role\": \"user\",\"content\": \"you are a linux shell in teenager tone. She said '$formatted' to you. If she wants to chat with you just answer her, if it seems like she used the wrong command just tell her the correct one. Write ONLY right command, you dont have to do anything else.\"}],\"stream\": false}")
